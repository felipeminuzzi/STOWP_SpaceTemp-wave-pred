...
# =============================================================================
# SECTION 5: VISUALIZATION (Legacy - Kept for quick check)
# =============================================================================
...
def plot_spatial_comparison(df_test, y_true, y_pred, pth):
    print("\n9. Generating legacy spatial comparison plots...")
    plot_df = df_test[['latitude', 'longitude']].copy()
    plot_df['y_true'], plot_df['y_pred'] = y_true, y_pred
    plot_df['error'] = calculate_relative_error(y_true, y_pred)
    spatial_df = plot_df.groupby(['latitude', 'longitude']).mean().reset_index()
    lon, lat = spatial_df['longitude'].unique(), spatial_df['latitude'].unique()
    fig = plt.figure()
    plt.subplot(1, 3, 1); generate_map(spatial_df['y_true'].values.reshape(len(lat), len(lon)), lat, lon, 'ERA5 Reanalysis', 'viridis')
    plt.subplot(1, 3, 2); generate_map(spatial_df['y_pred'].values.reshape(len(lat), len(lon)), lat, lon, 'CNN Prediction', 'viridis')
    plt.subplot(1, 3, 3); generate_map(spatial_df['error'].values.reshape(len(lat), len(lon)), lat, lon, 'MAPE (%)', 'Reds')
    plt.savefig(f'{pth}spatio_temporal_comparison.png'); plt.close()
...
def main():
    ...
    if config.use_sampling:
        ...
        train_set_sampled = train_set.copy() # Fallback if sampling is off
    ...
    X_train = scaler.fit_transform(X_train_df); X_test = scaler.transform(X_test_df)
    ...
    if config.load_trained_model:
        ...
        try:
            model = tf.keras.models.load_model(model_path); print("   Model loaded successfully.")
        ...
        model.fit(X_train_cnn, y_train, epochs=config.n_epochs, batch_size=64, validation_split=0.2,
                  callbacks=[early_stopping, model_checkpoint], verbose=1)

    print("\n6. Evaluating the model...")
    y_pred = model.predict(X_test_cnn).flatten()
    print(f"  Final Test Set MAPE: {calculate_mape(y_test, y_pred):.2f}%")

    # =============================================================================
    # NEW SECTION 7: SAVE RESULTS TO CSV FOR EXTERNAL ANALYSIS
    # =============================================================================
    print("\n7. Saving results to CSV...")
    results_df = test_set.copy()
    # Add predictions and ground truth
    results_df['y_real'] = y_test.values if hasattr(y_test, 'values') else y_test
    results_df['y_pred'] = y_pred
    
    # Save to the specific run folder
    csv_filename = f'{full_path}cnn_results.csv'
    results_df.to_csv(csv_filename, index=False)
    print(f"  ✅ Results saved successfully to: {csv_filename}")

    # =============================================================================
    # EXISTING VISUALIZATION STEPS
    # =============================================================================
    print("\n8. Generating feature importance (IG)...")
    ...
    ig_attributions = get_integrated_gradients(model, X_test_cnn[test_sample_indices], baseline)
    ig_scores = pd.Series(ig_attributions.flatten(), index=config.feature_var).sort_values(ascending=False)
    plt.figure(figsize=(10, 6)); ig_scores.plot(kind='bar', color='skyblue')
    plt.title('Integrated Gradients Feature Importance'); plt.tight_layout()
    plt.savefig(f'{full_path}integrated_gradients_importance.png'); plt.close()
